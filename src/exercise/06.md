...
- Each `React.useState` may cause a re-render and so it is wise to tie all
  states together as much as possible.
- Defining `ErrorBoundary` class is a way to custom handle all kinds of errors,
  including run-time errors thrown by a particular element.
- `key` prop is a way to make the component re-render at the node when value of
  the `key` changes.
- There is also
  [react-error-boundary](https://github.com/bvaughn/react-error-boundary#:~:text=%3CErrorBoundary%20fallback%3D%7B%3Cdiv%3ESomething%20went%20wrong%3C/div%3E%7D%3E%0A%20%20%3CExampleApplication%20/%3E%0A%3C/ErrorBoundary%3E)
  library which handles common errors.
- `key` prop unmounts and re-mounts the component everytime there is a change,
  so this is not ideal, better to use `resetErrorBoundary` and `onReset` exposed
  by the
  [react-error-boundary](https://github.com/bvaughn/react-error-boundary#:~:text=%3CErrorBoundary%20fallback%3D%7B%3Cdiv%3ESomething%20went%20wrong%3C/div%3E%7D%3E%0A%20%20%3CExampleApplication%20/%3E%0A%3C/ErrorBoundary%3E)
  library.
- `resetKeys` of
  [react-error-boundary](https://github.com/bvaughn/react-error-boundary#:~:text=%3CErrorBoundary%20fallback%3D%7B%3Cdiv%3ESomething%20went%20wrong%3C/div%3E%7D%3E%0A%20%20%3CExampleApplication%20/%3E%0A%3C/ErrorBoundary%3E)
  library is a nice way to re-render the component when any of the keys in the
  array change.
